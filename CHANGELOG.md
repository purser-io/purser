...
- **Validation benchmark, Phase 2 + gate:** a peer-scanner head-to-head comparison (`benchmarks/compare.py` vs picklescan/ModelScan/Fickling/ModelAudit), regression-gate flags on `run.py` (`--min-tpr` / `--max-fpr`), an expanded benign corpus, and a scheduled CI workflow (`benchmark.yml`) that fails on a detection drop or FPR rise. Published numbers in `benchmarks/README.md`.
