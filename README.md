...
```bash
helm install purser oci://ghcr.io/purser-io/charts/purser --version 0.1.3 \
...
```yaml
- uses: purser-io/purser@v0.1.3   # or pin a commit SHA
  with:
    path: ./models
...
| **Exfiltration engine** | *all files* | Webhook endpoints (Slack/Discord/Telegram), hard-coded IP:port, non-allowlisted URLs, cloud/API credentials (AWS, GitHub, HF, OpenAI, private keys, JWTs), embedded source with network/exec/shell idioms, base64/hex/**base32**/**base85**-encoded payloads (decoded and re-analyzed, incl. one **gzip/zlib** layer), and **UTF-16 (wide) strings** that hide indicators from ASCII scans. Scans in bounded windows with a per-file finding cap; benign-host allowlist is configurable/strict-able (see env table). |
...
docker run --rm -v $PWD/models:/models:ro -v $PWD/policies:/policies:ro \
  ...
  ghcr.io/purser-io/purser:0.1.3
# one-shot CLI scan:
docker run --rm -v $PWD/models:/models:ro \
  ghcr.io/purser-io/purser:0.1.3 purser scan /models
